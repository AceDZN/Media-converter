...
from app.services.converter import convert_pdf_sync, convert_pptx_sync
...
@router.post(
    ...
    description="Upload a PDF file and convert each page to a JPEG/PNG image.",
)
async def convert_pdf_to_image(
    ...
    if file_ext not in settings.allowed_pdf_extensions:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid file type '{file_ext}'. Allowed: {', '.join(settings.allowed_pdf_extensions)}",
        )
    ...
    try:
        ...
        logger.info(f"Received PDF file: {file.filename} ({total_size} bytes) -> Job {job_id}")

        # Submit conversion job
        job = await job_manager.submit_job(job_id, str(input_path), convert_pdf_sync)
        ...
        if wait:
            ...
            return ConversionResponse(
                job_id=job_id,
                status=JobStatus(job.status.value),
                message=f"Successfully converted {len(job.images)} pages",
                total_slides=len(job.images),
                images=job.images,
                processing_time_ms=job.processing_time_ms,
            )
        ...
        logger.exception(f"PDF conversion endpoint error for job {job_id}")
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
